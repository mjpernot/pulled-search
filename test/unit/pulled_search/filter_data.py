...
"""Program:  filter_data.py
    ...
    Description:  Unit testing of filter_data in pulled_search.py.

    Usage:
        test/unit/pulled_search/filter_data.py
...
class CfgTest(object):
    ...
    def __init__(self):
        ...
        self.raw_archive_dir = "/path/to/archive"
        self.unparsable_dir = "/path/to/unparsable"
...
class UnitTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.entry7 = ip1 + proxy + user + dtg2 + reqid + verb + stats \
            + ref + agt + url5
        self.entry8 = ip1 + proxy + user + dtg2 + reqid + verb + stats \
            + ref + agt + url
    ...
    def test_filter_successful(self, mock_log):

        """Function:  test_filter_successful

        Description:  Test with filter with successful pass.
    ...
    def test_url_no_docid(self, mock_log):
        ...
        self.assertEqual(
            pulled_search.filter_data(
                self.cfg, mock_log, self.log_json8)["servers"]["server_name"],
            self.results)
    ...
    def test_url_contains_2ndreview(self, mock_log):
        ...
        self.assertEqual(
            pulled_search.filter_data(
                self.cfg, mock_log, self.log_json7)["servers"]["server_name"],
            self.results)
    ...
    def test_url_contains_ic_gov(self, mock_log):
        ...
        self.assertEqual(
            pulled_search.filter_data(
                self.cfg, mock_log, self.log_json6)["servers"]["server_name"],
            self.results)
    ...
    def test_bad_status_code(self, mock_log):
        ...
        self.assertEqual(
            pulled_search.filter_data(
                self.cfg, mock_log, self.log_json5)["servers"]["server_name"],
            self.results)
    ...
    def test_partial_log_entry(self, mock_log):
        ...
        self.assertEqual(
            pulled_search.filter_data(
                self.cfg, mock_log, self.log_json4)["servers"]["server_name"],
            self.results)
